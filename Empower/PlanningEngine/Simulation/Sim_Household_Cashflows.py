...
import Simulation.Sim_Cashflow_Array_Creator as scar
import os
...
class Sim_Household_Cashflows(object):
    def __init__(self, household, init_params):
        ...
        self.hhc_dataframe = pd.concat(
            [self.hhc_dataframe, (pd.DataFrame(self.set_retirement_need_array(), columns=self.column_headers))])
        self.hhc_dataframe = pd.concat(
            [self.hhc_dataframe, (pd.DataFrame(self.set_ss_array(), columns=self.column_headers))])
        self.hhc_dataframe = pd.concat(
            [self.hhc_dataframe, (pd.DataFrame(self.set_pension_array(), columns=self.column_headers))])
        if self.household.cashflow_array != []:
            self.hhc_dataframe = pd.concat(
                [self.hhc_dataframe, (pd.DataFrame(self.set_cashflow_array(), columns=self.column_headers))])
    ...
    def set_compensation_array(self):
        ...
        for each_ind in self.household.head_of_household_array:
            for each_job in each_ind.job_array:
                temp_np = np.hstack((each_job.get_earnings_curve(each_ind.age, each_ind.retire_age),
                                     np.zeros(100 - (each_ind.retire_age - each_ind.age))))
                temp_compensation_array = np.vstack(
                    (temp_compensation_array, np.hstack((each_ind.name, each_job.name, 'earnings', temp_np))))
    ...
    def set_cashflow_array(self):
        ...
        for each_cashflow in self.household.cashflow_array:
            temp_np = scar.create_cashflow_array(each_cashflow.amount, each_cashflow.start_year - 2018,
                                                 each_cashflow.end_year - 2018, 100, self.init_params,
                                                 each_cashflow.inflate_to_start, each_cashflow.inflate_to_end,
                                                 each_cashflow.inflation)

            temp_cashflow_array = np.vstack((temp_cashflow_array,
                                             np.hstack(('houeshold', each_cashflow.name, 'cashflow', temp_np))))
    ...
    def set_retirement_need_array(self):
        ...
        for each_ind in self.household.head_of_household_array:
            if each_ind.retirement_need_type == 'dollar':
                ...
                temp_retirement_need = each_ind.retirement_need * self.get_earnings(each_ind.name, int(
                    each_ind.retire_age - each_ind.age - 1))
            ...
            temp_retire_need_array = np.vstack((temp_retire_need_array,
                                                np.hstack(
                                                    (each_ind.name, 'retirement_need', 'retirement_need', temp_np))))
        ...
        if self.household.number_of_hoh == 2:
            first_persion_death_period = int(
                self.household.head_of_household_array[0].life_expectancy - self.household.head_of_household_array[
                    0].age)
            second_person_death_period = int(
                self.household.head_of_household_array[1].life_expectancy - self.household.head_of_household_array[
                    1].age)
            ...
            post_death_retire_need = (
                (float(temp_retire_need_array[0, int(first_persion_death_period + self.start_earnings_column - 1)])
                 + float(temp_retire_need_array[1, int(first_persion_death_period + self.start_earnings_column - 1)]))
                * config.retirement_need_adjustment_after_death
            )
            ...
            if second_person_death_period > first_persion_death_period:
                temp_retire_need_array[hoh_row_to_change,
                first_persion_death_period + self.start_earnings_column:second_person_death_period + self.start_earnings_column] = (
        ...
        # print (temp_retire_need_array)
        self.include_in_annual_net_numpy = np.vstack(
            (self.include_in_annual_net_numpy, temp_retire_need_array[:, self.start_earnings_column:])).astype(float)
    ...
    def set_ss_array(self):
        ...
        for each_ind in self.household.head_of_household_array:
            temp_np = scar.create_cashflow_array(each_ind.ss_benefit, each_ind.ss_start_age - each_ind.age,
                                                 each_ind.life_expectancy - each_ind.age, 100, self.init_params)
            temp_ss_array = np.vstack(
                (temp_ss_array, np.hstack((each_ind.name, 'social_security', 'social_security', temp_np))))
    ...
    def set_pension_array(self):
        ...
        for each_ind in self.household.head_of_household_array:
            for each_pension in each_ind.pension_array:
                temp_np = scar.create_cashflow_array(each_pension.amount, each_pension.start_age,
                                                     each_ind.life_expectancy, 100, self.init_params,
                                                     each_pension.inflate_to_start, each_pension.inflate_to_end,
                                                     each_pension.inflation)

                temp_pension_array = np.vstack(
                    (temp_pension_array, np.hstack((each_ind.name, each_pension.name, 'pension', temp_np))))
    ...
    def get_earnings(self, ind_name='', period_number='', job_name=''):
        ...
        if ind_name == '':
            ...
            temp_df = (self.hhc_dataframe.loc[
                           (self.hhc_dataframe['category'] == "earnings") & (self.hhc_dataframe['owner'] == ind_name)])

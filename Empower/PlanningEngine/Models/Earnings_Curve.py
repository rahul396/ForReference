...
# used to adjust future earnigns based on growth assumptions by age and earnings ID
...
class Earnings_Curves(object):
    def __init__(self, file_name_with_ext, ip_sheet_num, error_init):
        ...
        self.earnings_df = self.xlfile.parse(ip_sheet_num,
                                             header=0)  # Always reads in teh first tab of the excel file, could change by chaging first zero
        # minimum age in the data source
        self.min_supported_age = self.earnings_df.iloc[:, 0].min()
        # maximum age in the data source
        self.max_supported_age = self.earnings_df.iloc[:, 0].max()
    ...
    def get_earnings_growth_curve(self, earnings_curve_id='default', start_age='', end_age=''):
        ...
        try:
            ...
            self.error_init.add_error('warning', "Earnings Curve - get_earnigns_growth_curve",
                                      "could not find earnings growth curve " + earnings_curve_id + ", used default instead")

        # assume the starting point is the first row, ending point is the last end_row
        start_row = 0
        ...
        if start_age != '':
            ...
            if start_age < self.min_supported_age:
                self.error_init.add_error('critical', 'Earnings_Curves - get_earnings_curve', 'start age requested,  ',
                                          start_age, ', is less then min age supported, ', self.min_supported_age)

            start_row = np.where(
                self.earnings_df.iloc[:, 0] == start_age)[0][0]
        ...
        if end_age != '':
            ...
            if end_age > self.max_supported_age:
                self.error_init.add_error('critical', 'Earnings_Curves - get_earnings_curve', 'end age requested',
                                          'max_age is greater then max age supported, ', self.max_supported_age)

            end_row = np.where(self.earnings_df.iloc[:, 0] == end_age)[0][0]

        return np.transpose(np.vstack(
            (self.earnings_df.iloc[start_row:end_row, 0], self.earnings_df.iloc[start_row:end_row, column_index])))
    ...
    def get_real_earnings(self, start_age, current_earnings, earnings_curve_id='default', end_age=''):
        ...
        earnings_growth_array = self.get_earnings_growth_curve(
            earnings_curve_id, start_age, end_age)
        ...
        for age_index in range(1, np.shape(earnings_growth_array)[0]):
            real_earnings_array[age_index] = real_earnings_array[age_index - 1] * (
                1 + earnings_growth_array[age_index - 1, 1])

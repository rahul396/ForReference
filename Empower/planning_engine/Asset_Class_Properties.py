...
class Asset_Class_Properties(object):
    ...
    def __init__ (self, file_path_and_name_with_ext, ip_sheet_num, error_init):
        ...
        self.acp = np.hstack((self.acp,np.array(xlsheet.iloc[:,start_covar_col:self.end_covar_col])))
    ...
    def __get_multiple_rows(self,asset_class_name_array, return_col):
        temp =  self.acp[np.where(self.acp[:,0]== asset_class_name_array[0]),return_col]
        for ac_iter  in range(1,np.shape(asset_class_name_array)[0]):
            temp = np.vstack((temp,self.acp[np.where(self.acp[:,0]== asset_class_name_array[ac_iter]),return_col]))
        return temp

    def get_lowest_risk_asset_class(self):
        return self.acp[np.where(self.acp[:,stdev_col] ==(np.min(self.acp[:,stdev_col]))),asset_class_name_col][0][0]
    ...
    def get_covariance_matrix(self, asset_class_name_array = []):
        if asset_class_name_array == []:
            ...
            for ac_iter  in range(num_requested_assets):
                cov_index_array[ac_iter] =  np.where(self.acp[:,0]== asset_class_name_array[ac_iter])[0]

            final_cov_array = np.empty([num_requested_assets,num_requested_assets])
            
            for x in range(num_requested_assets):
                for y in range(num_requested_assets):
                    final_cov_array[x,y] = self.acp[int(cov_index_array[x]),int(cov_index_array[y]+start_covar_col )]

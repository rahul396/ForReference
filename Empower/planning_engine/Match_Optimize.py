...
def get_optimized_match(fms_array, total_contribution, init_params):
    ...
    for fms_item in sorted_fms_array:
        if running_contribution + fms_item.match_limit <= total_contribution:
            temp_match_array.append([fms_item.match_limit,fms_item.account_type, fms_item.job_name, fms_item.owner_name, fms_item.earnings])
            running_contribution += fms_item.match_limit
        elif running_contribution < total_contribution:
            temp_match_array.append([total_contribution - running_contribution,fms_item.account_type, fms_item.job_name, fms_item.owner_name, fms_item.earnings])

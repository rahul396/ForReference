...
class Flat_Match_Structure(object):
    ...
    def __init__(self,match_amount, match_limit, match_type, account_type, contribution_limit, earnings = "", job_name = "", owner_name = ""):
        ...
        self.append_row(match_amount, match_limit, match_type, account_type, contribution_limit, earnings, job_name, owner_name)

    def append_row (self,match_amount, match_limit, match_type, account_type, contribution_limit, earnings = "", job_name = "", owner_name = ""):
    ...
    def adjust_match_for_salary_and_limit(self):
        ...
        for row_iter in range(num_rows):
            running_total = 0
            limit_reached = False
            salary_or_plan_limit = min(sorted_fms.loc[row_iter,"contribution_limit"],sorted_fms.loc[row_iter,"earnings"])
            temp_id = sorted_fms.loc[row_iter,"owner_name"] + sorted_fms.loc[row_iter,"job_name"] + sorted_fms.loc[row_iter,"account_type"]

            while temp_id == sorted_fms.loc[row_iter,"owner_name"] + sorted_fms.loc[row_iter,"job_name"] + sorted_fms.loc[row_iter,"account_type"]:
                if limit_reached:

...
class Mortality(object):
    ...
    def __init__(self, file_path_and_name_with_ext, ip_sheet_number,error_init):
        ...
        self.temp_gm_gb_array = np.array(xlsheet.iloc[:,0:5])#temp becuase in 5 year increments, adjustment below
    ...
    def __expand_gm_gb_table(self):
        ...
        for row_iter in range(self.temp_gm_gb_array.shape[0] - 1):
            if (self.temp_gm_gb_array[row_iter,0] == self.temp_gm_gb_array[row_iter+1,0] and self.temp_gm_gb_array[row_iter,1] == self.temp_gm_gb_array[row_iter+1,1] and self.temp_gm_gb_array[row_iter,2] != self.temp_gm_gb_array[row_iter+1,2]):
                ...
                for age_iter in range(start_age, end_age):
                    ...
                    new_gm = self.temp_gm_gb_array[row_iter,3] * lower_weight + self.temp_gm_gb_array[row_iter+1,3] * upper_weight
                    new_gb = self.temp_gm_gb_array[row_iter,4] * lower_weight + self.temp_gm_gb_array[row_iter+1,4] * upper_weight
                    new_row = np.array([self.temp_gm_gb_array[row_iter,0],self.temp_gm_gb_array[row_iter,1], age_iter,new_gm, new_gb ])
                    expanded_gm_gb_array = np.vstack((expanded_gm_gb_array,new_row))

            else:
                expanded_gm_gb_array = np.vstack((expanded_gm_gb_array, self.temp_gm_gb_array[row_iter,:]))

        #add the last row to the array, have to do it here becuase of the index + 1 above
        expanded_gm_gb_array = np.vstack((expanded_gm_gb_array, self.temp_gm_gb_array[-1,:]))
        #for some reason everything is a string except the last row so do this to make sure all rows are of the same type
        expanded_gm_gb_array = (np.hstack((expanded_gm_gb_array[:,0:2],expanded_gm_gb_array[:,2:3].astype('str') ,expanded_gm_gb_array[:,3:5].astype('float'))))
    ...
    def get_gm_gb_factors(self, health_state = 'all', gender = 'all', age = 'all'):
        ...
        if health_state =='all' and gender == 'all' and age == 'all':
            ...
            temp_array =  self.gm_gb_array[np.where(self.gm_gb_array[:,2]== age),:]

        elif health_state =='all' and age == 'all':
            temp_array =  self.gm_gb_array[np.where(self.gm_gb_array[:,1]== gender),:]

        elif gender =='all' and age == 'all':
            temp_array =  self.gm_gb_array[np.where(self.gm_gb_array[:,0]== health_state),:]

        elif health_state == 'all':
            temp_array =  self.gm_gb_array[np.where((self.gm_gb_array[:,1] == gender) & (self.gm_gb_array[:,2] == age))]

        elif gender == 'all':
            temp_array =  self.gm_gb_array[np.where((self.gm_gb_array[:,0] == health_state) & (self.gm_gb_array[:,2] == age))]

        elif age =='all':
            temp_array =  self.gm_gb_array[np.where((self.gm_gb_array[:,0] == health_state) & (self.gm_gb_array[:,1] == gender))]

        else:
            temp_array =  self.gm_gb_array[np.where((self.gm_gb_array[:,0] == health_state) &
            (self.gm_gb_array[:,1] == gender) & (self.gm_gb_array[:,2] == age)),3:5][0]
    ...
    def mortality_prob_calc(self,gender, start_age, health_state = 'good', end_age = 110):
        ...
        if start_age < self.min_age:
            ...
            temp_underage_mortality_array = np.full((self.min_age - start_age),1)

        period_array = np.arange(0,number_of_periods)
        age_array = np.arange(gm_start_age,end_age)

        gm_gb_array = self.get_gm_gb_factors(health_state = health_state ,gender = gender,age = gm_start_age)
        gm = float(gm_gb_array[:,0][0])
        gb = float( gm_gb_array[:,1][0])

        temp_mortality_array = self.gompertz_calc(gm, gb, gm_start_age, np.arange(1,number_of_periods+1))
    ...
    def get_unisex_mortality(self,mortality_array_one, mortality_array_two):
        temp_mortality_array = np.array([mortality_array_one,mortality_array_two])
